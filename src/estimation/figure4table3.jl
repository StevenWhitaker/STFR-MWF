...
Create Figure 4 and Table 3. Figure 4 and Table 3 compare various MWF estimation
techniques using white matter and gray matter tissues with a three-compartment
model with exchange.
Figure 4 also displays the B0 and B1+ maps used in the simulation.
"""
function figure4table3()

    # Run figure4table3data() if it hasn't run yet
    (isfile(modulepath("estimation/results/brainweb_STFR2-PERK.jld")) &&
     isfile(modulepath("estimation/results/brainweb_STFR3-PERK.jld")) &&
     isfile(modulepath("estimation/results/brainweb_MESE-NNLS.jld")) &&
     isfile(modulepath("estimation/results/brainweb_MESE-PERK.jld")) &&
     isfile(modulepath("estimation/results/brainweb_STFR3-PERK-JE.jld"))) || figure4table3data()

    (ff1, ffmap1, rmse_wm1, rmse_gm1, mean_wm1, mean_gm1, std_wm1, std_gm1, t1) =
        load(modulepath("estimation/results/brainweb_STFR2-PERK.jld"), "ff", "ffmap",
        "rmse_wm", "rmse_gm", "mean_wm", "mean_gm", "std_wm", "std_gm", "t")
    (ff2, ffmap2, rmse_wm2, rmse_gm2, mean_wm2, mean_gm2, std_wm2, std_gm2, t2) =
        load(modulepath("estimation/results/brainweb_STFR3-PERK.jld"), "ff", "ffmap",
        "rmse_wm", "rmse_gm", "mean_wm", "mean_gm", "std_wm", "std_gm", "t")
    (ff3, ffmap3, rmse_wm3, rmse_gm3, mean_wm3, mean_gm3, std_wm3, std_gm3, t3) =
        load(modulepath("estimation/results/brainweb_MESE-NNLS.jld"), "ff", "ffmap",
        "rmse_wm", "rmse_gm", "mean_wm", "mean_gm", "std_wm", "std_gm", "t")
    (ff4, ffmap4, rmse_wm4, rmse_gm4, mean_wm4, mean_gm4, std_wm4, std_gm4, t4) =
        load(modulepath("estimation/results/brainweb_MESE-PERK.jld"), "ff", "ffmap",
        "rmse_wm", "rmse_gm", "mean_wm", "mean_gm", "std_wm", "std_gm", "t")
    (ff5, ffmap5, rmse_wm5, rmse_gm5, mean_wm5, mean_gm5, std_wm5, std_gm5, t5) =
        load(modulepath("estimation/results/brainweb_STFR3-PERK-JE.jld"), "ff",
...
Create data for Figure 4 and Table 3.
"""
function figure4table3data()
    ...
    (data, obj, mask) = testdata_brainweb(:stfrblochsim, Δω = Δω, κ = κ, σ = σ)
    ...
    @save(modulepath("estimation/results/brainweb_STFR2-PERK.jld"),
    ...
    @save(modulepath("estimation/results/brainweb_STFR3-PERK.jld"),
        ff, ffmap, rmse_wm, rmse_gm, mean_wm, mean_gm, std_wm, std_gm, t)
    ...
    @save(modulepath("estimation/results/brainweb_STFR3-PERK-JE.jld"),
        ff, ffmap, rmse_wm, rmse_gm, mean_wm, mean_gm, std_wm, std_gm, t)

    # MESE-PERK
    Random.seed!(20191204)
    (data, obj, mask) = testdata_brainweb(:mese, Δω = Δω, κ = κ, σ = σ)
    Nnoise = count(getmask(obj, :background))
    noise = testdata_noise(N = Nnoise, σ = σ)
    ...
    @save(modulepath("estimation/results/brainweb_MESE-PERK.jld"),
        ff, ffmap, rmse_wm, rmse_gm, mean_wm, mean_gm, std_wm, std_gm, t)
    ...
    @save(modulepath("estimation/results/brainweb_MESE-NNLS.jld"),
        ff, ffmap, rmse_wm, rmse_gm, mean_wm, mean_gm, std_wm, std_gm, t)
